...
function imageContent(base64: string, mimeType = 'image/jpeg') {
  return {
    type: 'image' as const,
    source: {
...
/* ─── Existing: single-image analysis (kept for demo seeds + compare) ─── */
...
export async function analyzeCarImage(imageBase64: string, mimeType = 'image/jpeg'): Promise<AIAnalysisResult> {
  const response = await getClient().messages.create({
    ...
    messages: [{
      role: 'user',
      content: [imageContent(imageBase64, mimeType), { type: 'text', text: DAMAGE_ANALYSIS_PROMPT }],
    }],
  })
...
export async function compareInspections(
  ...
  const response = await getClient().messages.create({
    ...
    messages: [{
      role: 'user',
      content: [imageContent(postImageBase64, mimeType), { type: 'text', text: prompt }],
    }],
  })
  const text = response.content[0].type === 'text' ? response.content[0].text : ''
...
/* ─── NEW: Pipeline Step 1 — Claude Haiku direct analysis (no YOLO) ──── */
...
export async function analyzeFrameDirect(imageBase64: string): Promise<DirectAnalysisResult> {
  const response = await getClient().messages.create({
    ...
    messages: [{
      role: 'user',
      content: [imageContent(imageBase64), { type: 'text', text: DIRECT_ANALYSIS_PROMPT }],
    }],
  })
  const text = response.content[0].type === 'text' ? response.content[0].text : ''
  try {
...
/* ─── NEW: Pipeline Step 2 — Claude Haiku panel identification (with YOLO) */

export interface PanelIdentification {
  angle: string
  primaryPanel: string
  detectionPanels: string[]
  visiblePanels: string[]
}
...
Return ONLY this JSON (no other text):
{
  ...
  const response = await getClient().messages.create({
    ...
    messages: [{
      role: 'user',
      content: [imageContent(imageBase64), { type: 'text', text: prompt }],
    }],
  })
  const text = response.content[0].type === 'text' ? response.content[0].text : ''
  try {
...
/* ─── NEW: Pipeline Step 3 — Claude Sonnet damage verification ───────── */
...
REAL damage: scratches, dents, cracks, paint chips, rust, broken/missing parts, repainted panels, panel misalignment, rim/wheel damage, foggy/cracked headlights.
FALSE POSITIVES to reject: reflections on paint, stickers or decals, intentional aftermarket body mods, dirt, dust, water drops, shadows, UNIFORM factory panel gaps (only flag uneven gaps), tyre SIDEWALL rubber scuffs (only flag metal rim damage), tinted windows.
...
export interface FinalReport {
  ...
  totalRepairCostPKR: number
  totalRepairCostMin: number
  totalRepairCostMax: number
